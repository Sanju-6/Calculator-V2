...
calculator.setAttribute(
  "style",
  "width:405px; height:620px; display:flex; justify-content:space-evenly; flex-wrap:wrap; align-items:stretch; overflow:hidden; margin-top:50px;"
);
display.setAttribute(
  "style",
  "width:402px; border: 1px solid #4a4a4a; height:140px; display:flex; align-items:center"
);
display.setAttribute("class", "notification");
displayContent.setAttribute("class", "title");
divider.setAttribute(
  "style",
  "width: 402px; height:50px; display:flex; justify-content:flex-end;"
);
buttons.setAttribute(
  "style",
  "width: 410px; height:400px; display:flex; flex-direction:colums; justify-content:space-evenly"
);
...
for (let item of operators) {
  allOtherButtons.appendChild(item);
}
...
for (let item of operators) {
  item.addEventListener("click", calcDisplay);
}
...
function calcDisplay(e) {
  ...
  if (!isNaN(individualInput) || individualInput == ".") {
    ...
    if (
      ...
      alert(
        "There already exists a decimal in the equation. Please check the equation and try again."
      );
      return;
    } else {
      strInput = strInput + individualInput;
...
function backSpace(e) {
  if (
    !isNaN(strInput.charAt(strInput.length - 1)) &&
    strInput.charAt(strInput.length - 1) != " "
  ) {
